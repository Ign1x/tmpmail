...
| Task | Location | Notes |
|---|---|---|
| Bring the stack up/down | `compose.yaml`, `scripts/dev-up.sh`, `scripts/dev-down.sh` | Default workflow is plain `docker compose`; `dev-up` can auto-create `.env`, ask for script language, prompt for required values plus proxy mode, hard-fail when host `25/TCP` is not both published to Inbucket and reachable on `127.0.0.1:25`, hard-fail when `TMPMAIL_MAIL_EXCHANGE_HOST:25` is unreachable, wait for `api` and `frontend` readiness, print DNS follow-up, and then optionally prompt to clean TmpMail-owned dangling Docker images |
