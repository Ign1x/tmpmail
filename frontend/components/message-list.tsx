...
export default function MessageList({
  ...
  return (
    <div
      className={`h-full w-full overflow-y-auto ${isMobile ? "p-2" : "p-4"}`}
    >
      <div className={`${isMobile ? "mb-4" : "mb-6"} w-full`}>
        <h2
          className={`${isMobile ? "text-xl" : "text-2xl"} font-bold text-gray-800 dark:text-gray-100`}
        >
          {t("inbox")}
        </h2>
        <div
          className={`flex items-center gap-2 text-xs text-gray-500 ${isMobile ? "mt-1" : "mt-2"} ${isMobile ? "flex-wrap" : ""}`}
        >
          <div
            ...
            <CardBody className={`${isMobile ? "p-3" : "p-5"} w-full`}>
              ...
              >
                <div className="relative">
                  ...
                  {!message.seen && (
                    <div
                      className={`absolute -top-1 -right-1 ${isMobile ? "w-2.5 h-2.5" : "w-3 h-3"} bg-primary-500 border-2 border-white dark:border-gray-800 rounded-full`}
                    ></div>
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    ...
                    <div
                      className={`flex flex-col items-end ${isMobile ? "ml-2" : "ml-3"}`}
                    >
                      <span
                      ...
                      {!message.seen && (
                        <div
                          className={`${isMobile ? "mt-0.5 px-1.5 py-0.5" : "mt-1 px-2 py-0.5"} bg-primary-500 text-white text-xs rounded-full font-medium`}
                        >
                          {t("new")}
                        </div>
                      )}
                  ...
                  <p
                    className={`${isMobile ? "text-xs" : "text-sm"} leading-relaxed line-clamp-2 ${
                      !message.seen
